module Spree
  class InquiryMailer < ActionMailer::Base
    default :from => Spree::MailMethod.current.preferred_mails_from if Spree::MailMethod.current

    def notification(inquiry, customer = false)
      @inquiry = inquiry

      recipients = Spree::MailMethod.current ? (Spree::MailMethod.current.preferred_intercept_email || Spree::MailMethod.current.preferred_mail_bcc.split) : ""
      ...
      mail(
        ...
        :from => customer && Spree::MailMethod.current ? Spree::MailMethod.current.preferred_mails_from : inquiry.email,
